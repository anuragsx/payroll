class Calculator < ActiveRecord::Base
  ...
  validates :name, :type, :calculator_type, :presence => true

  attr_accessor :company, :employee, :salary_sheet, :salary_slip, :run_date

  #scope :by_leave, :conditions => ["calculator_type = ?",'Leave']
  #scope :by_package, :conditions => ["calculator_type = ?",'Package']
  #scope :by_allowance, :conditions => ["calculator_type = ?",'Allowance']
  #scope :by_deduction, :conditions => ["calculator_type = ?",'Deduction']
  #scope :by_subtotal, :conditions => ["calculator_type = ?",'Subtotal']
  #scope :by_addon, :conditions => ["calculator_type = ?",'Addon']
  #scope :in_type, lambda{|d| {:conditions => ["calculators.type in (?)",d]}}
